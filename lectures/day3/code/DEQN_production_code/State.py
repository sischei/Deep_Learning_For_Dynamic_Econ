...
def monomial_rule(scale_list):
    d = len(scale_list)
    shock_probabilities = tf.constant([1 / (2*d) ] * (2*d),dtype=global_default_dtype)
    shock_values = tf.constant(np.concatenate((np.diag(scale_list) * math.sqrt( d / 2.0), np.diag(scale_list) * -math.sqrt( d / 2.0))), dtype=global_default_dtype)
...
# computes the expectation in double precision. Can be used if the rest of the model is single-precision, 
# but high accuracy is used in the Expectation. Can also be used only in the simulation/postprocessing
# by commenting out/in the respective function in the Equations.
def E_t_gen_double(state, policy_state):
    if expectation_type != 'pseudo_random':
        next_states = [Dynamics.total_step_spec_shock(state, policy_state, i) for i in range(len(Dynamics.shock_probs))]
        next_policies = [policy(next_state) for next_state in next_states]
        ...
        def E_t(evalFun):
            # calculate conditional expectation
            res = tf.zeros(state.shape[0],dtype=tf.float64)  
            for i in range(len(Dynamics.shock_probs)):
                res += tf.cast(Dynamics.shock_probs[i],dtype=tf.float64) * tf.cast(evalFun(next_states[i], next_policies[i]),dtype=tf.float64)
            return res
    else:
        next_states = [Dynamics.total_step_random(state, policy_state) for i in range(expectation_pseudo_draws)]
        next_policies = [policy(next_state) for next_state in next_states]
        ...
        def E_t(evalFun):
            ...
            res = tf.zeros(state.shape[0],dtype=tf.float64)  
            for i in range(expectation_pseudo_draws):
                res += tf.cast(1.0 / expectation_pseudo_draws,dtype=tf.float64) * tf.cast(evalFun(next_states[i], next_policies[i]),dtype=tf.float64)
            return res
    return E_t
